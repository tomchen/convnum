...
describe('Date string parsing and formatting', () => {
  describe('parseDateString function', () => {
    describe('Y-M-D format variants', () => {
      test('should parse Y-M1-D1 format with dash separator', () => {
        ...
        expect(new Date(result.interpretations[0].timestamp)).toEqual(
          new Date(2023, 11, 25),
        )
      })
    ...
    describe('Edge cases and validation', () => {
      test('should handle leap year dates', () => {
        ...
        const feb29Interp = result.interpretations.find(
          (i) => i.format === 'Y-M2-D1',
        )
  ...
  describe('formatDateString function', () => {
    ...
    describe('Error handling', () => {
      test('should throw error for invalid format', () => {
        expect(() => formatDateString(testTimestamp, 'invalid-format')).toThrow(
          'Invalid format',
        )
      })

      test('should throw error for unknown format component', () => {
        expect(() => formatDateString(testTimestamp, 'Y-X-D1')).toThrow(
          'Invalid format component',
        )
      })

      test('should throw error for invalid day format', () => {
        expect(() => formatDateString(testTimestamp, 'Y-M1-D3')).toThrow(
          'Invalid day format',
        )
      })
    })
  ...
  describe('Round-trip conversion', () => {
    ...
    describe('Ambiguous formats', () => {
      ...
      ambiguousTestCases.forEach(
        ({ input, expectedFormats, expectedDates }) => {
          test(`should detect multiple interpretations for "${input}"`, () => {
            ...
            expectedDates.forEach((date) =>
              expect(timestamps).toContain(date.getTime()),
            )
            ...
            for (const interpretation of parsed.interpretations) {
              ...
              const matchingInterp = reparsed.interpretations.find(
                (i) =>
  ...
  describe('Integration with various timestamps', () => {
    ...
    test('should handle different years', () => {
      ...
      testCases.forEach(({ input, year }) => {
        ...
        const hasExpectedYear = result.interpretations.some(
          (i) => new Date(i.timestamp).getFullYear() === year,
        )
    ...
    test('should handle all months', () => {
      for (let month = 1; month <= 12; month++) {
        ...
        const hasExpectedMonth = result.interpretations.some(
          (i) => new Date(i.timestamp).getMonth() === month - 1,
        )
    ...
    test('should demonstrate practical ambiguity resolution', () => {
      ...
      const jan5Interp = result.interpretations.find(
        (i) => i.format === 'Y-M2-D2',
      )
      const may1Interp = result.interpretations.find(
        (i) => i.format === 'Y-D2-M2',
      )
